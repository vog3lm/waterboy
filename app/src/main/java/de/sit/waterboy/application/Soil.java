...
class Soil extends Water {

    Soil(Mode mode){
        super(mode);
        ActionBar bar = super.listing.getSupportActionBar();
        if(null != bar){bar.setTitle("Soil");}
    }
    @Override
    void onPrimary(){
    ...
    protected void onColorize(CheckBox box, Model model, float level, int normal, int urgent, int danger){
        if(0 > model.sc){box.setButtonTintList(ColorStateList.valueOf(urgent));}
        else if(level*model.si > model.sc){box.setButtonTintList(ColorStateList.valueOf(danger));}
    ...
    protected void onHide(){
        ...
        for(int i=0; i<listing.getChildCount()-1; i++){
            ...
            if(level*model.si < model.sc){item.setVisibility(View.GONE);}
        }
    }
}
